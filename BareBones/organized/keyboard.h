   ...
   if (!(keycode < 0 || (keycode & 0x80) != 0)){
    switch(keycode){
      ...
      case 0x1F:
        ...
        // make the compiler stop whining about intentional fallthrough
      ...
      case 0x19:
        if(ctrl){
          ...
          paddle_2_x = (80-pong_width_offset_bc_reg_width_breaks_everything)-1;
          disable_cursor();
          break;
        } 
        __attribute__((fallthrough));
      default:
        if(shift){
          terminal_putchar(shift_keycode_table[(int)keycode]);
        } else if(caps){
          terminal_putchar(caps_keycode_table[(int)keycode]);
        } else {
          terminal_putchar(keycode_table[(int)keycode]);
        }
    }
   }
   lastkeycode=keycode;
